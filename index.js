...
const feedbackCol = mongoose.connection.collection('feedbacks');
aws.config.update({
...
app.post('/send-api/feedback', async (req, res) => {
    try {
        ...
        const exst = await feedbackCol.findOne({ email, feedback });
        if (!exst)
        feedbackCol.insertOne({ email, feedback });
        const content = Object.entries(feedback).reduce((acc, [key, value]) => {
        ...
        const mailOptions = {
            ...
            to: email,
            subject: 'Feedback',
            text: `Feedback Submitted by User (${email})\n` + content,
        };
        ...
        console.log( `Feedback Submitted by User (${email})\n` + content)
    } catch (error) {
        console.log(error);
